...
case $ready in
 Y) echo "Installing the necessary, please wait..." && sudo xterm -e sudo apt update && apt install xen-hypervisor-4.11-amd64 xen-hypervisor-common xen-tools gparted xterm vinagre -y &
esac
...
case $dl in N) echo "First you have to make sure you have enough space to make a new volume group";
sudo gparted;
esac
...
then sed -i~ -e "s|/dev/sda6|server => \'${loc}\',|g" xen.cfg;
sudo vgcreate ${name} ${loc};
fi 
...
read -p "Time to reboot your machine are you ready? (Make sure download is finished) [ Y ]" ready;
case $ready
in Y) echo "Once you are booted make sure you sre booting with xen and rerun this script for the second part of the setup. Rebooting in 15 seconds..." && sleep 15 && sudo reboot now;;
esac
esac
